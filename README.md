...
**現在の既定構成：Claude を4ロール（提案・検証・実装・まとめ）に分けて相互チェック。**
ロールごとに使うAIは差し替え可能で、`OPENAI_API_KEY`（ChatGPT）や `GEMINI_API_KEY`（Gemini）を足せば**他社AIとの混成会議**にもできます。

▶ **会議が成果物をまとめ上げるまでの自動再生デモ**（動画のようなリプレイ）:
https://claude.ai/code/artifact/d0dac6d9-16c7-437c-8341-a714abd3bb44

### 会議ルーム（AI同士が円卓で会議・相互チェック）

![会議ルーム](docs/screenshot-meeting.png)

上は「店舗マニュアルAIの初回リリース仕様」を4ロールで相互チェックした例。プランナー（提案役）が範囲を提案 → クリティック（検証役）が「回答が見つからない時の挙動が未定義」という穴を指摘 → ビルダー（実装役）が具体化 → ファシリテーター（まとめ役）が論点を集約して承認、という流れがそのまま記録されています。
...
- 🤖 **複数AI（複数ロール）の会議** — 既定は Claude を4ロール（提案・検証・実装・まとめ）に分けて発言。ChatGPT・Gemini も混ぜられる
- 🔁 **3つの進め方**
